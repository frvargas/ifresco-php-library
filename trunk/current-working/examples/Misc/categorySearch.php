 ...
 */
    ...
    echo "<h2>GET Categories via REST:</h2>";
    
    $RestCategory = new RESTCategories($repository,$spacesStore,$session);
    $Categories = $RestCategory->GetCategories();
    echo "<pre>";
    print_R($Categories);
    echo "</pre>";      
    
    $categorySearch = "Languages";
    
    echo "<h2>SEARCH FOR ".$categorySearch."</h2><br>";

    $CategoryNode = $session->query($spacesStore, "PATH:\"cm:categoryRoot/cm:generalclassifiable/cm:" .$categorySearch. "\"");
    if ($CategoryNode != null) {
        echo "<pre>";
        print_R($CategoryNode);
        echo "</pre>";
    }
