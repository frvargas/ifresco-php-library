...
class RESTSites extends BaseObject {
    ...
    public function GetSites($userName="",$size="",$position="",$format="json") {
        $result = array();
        $userName = rawurlencode($userName);
        $size = rawurlencode($size);
        $position = rawurlencode($position);
        
        $this->_restClient->createRequest($this->_connectionUrl."/api/people/$userName/sites?size=$size&pos=$position&format=$format","GET");
